...
class DPTHead(nn.Module):
    ...
    def f1(self, encoder_features, teacher_dpt_features, patch_h, patch_w, fuse_fn,
                            temporal_layer, mamba_fn, **kwargs
                        ):
        '''
        all teacher_dpt_features are [B, 256, h', w'] (h, w=296 if image size is 518)
    ...
    def f2(self, path_4, encoder_features, teacher_dpt_features, patch_h, patch_w, fuse_fn,
                            temporal_layer, mamba_fn, **kwargs
                        ):
        out = []
        for i, x in enumerate(encoder_features):
