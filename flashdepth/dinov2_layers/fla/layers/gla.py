...
# a cleaner version where I remove all the if-else statements given the args I'm using
class DEBUG(nn.Module):
    def __init__(self):
        ...
        self.num_heads = 4
        self.num_kv_heads = 4  # since num_kv_heads defaults to num_heads
        self.num_kv_groups = 1  # num_heads // num_kv_heads = 1
        
        self.key_dim = int(1024 * 0.5)  # hidden_size * expand_k
        self.value_dim = int(1024 * 1.0)  # hidden_size * expand_v
        self.key_dim_per_group = self.key_dim
        self.value_dim_per_group = self.value_dim
        ...
        self.gk_proj = nn.Sequential(
            nn.Linear(self.hidden_size, 16, bias=False),  # gate_low_rank_dim
            nn.Linear(16, self.key_dim_per_group, bias=True)
        )
        self.o_proj = nn.Linear(self.value_dim, self.hidden_size, bias=False)

        self.g_norm_swish_gate = FusedRMSNormSwishGate(self.head_v_dim, True, 1e-5)
    ...
    def forward(
        ...
        **kwargs
        ...
        gk = rearrange(gk, 'b t (h d) -> b t h d', h=self.num_kv_heads)
